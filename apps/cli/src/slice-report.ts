import type { SliceAnalysis, SliceCriterion } from "@wrestling/sim";
...
function stories(analysis: SliceAnalysis): string {
  ...
  return `<div class="story-list">${analysis.stories.map((story) => {
    ...
    return `<article class="story"><b>${html(participants)}</b><span>${html(started)} → ${html(ended)}</span><p>${html(story.description)}</p></article>`;
  }).join("")}</div>`;
}
...
function pleCards(analysis: SliceAnalysis): string {
  ...
  return `<div class="cards">${analysis.pleCards.map((card) => `<article class="ple-card"><h3>Week ${card.week}</h3>${card.matches.map((match) => `<div class="match"><span>${html(match.position.replaceAll("_", " "))}</span><b>${html(match.participants.map((id) => wrestlerName(analysis, id)).join(" vs. "))}</b>${match.titleId ? "<i>Title</i>" : ""}${match.storyId ? "<i>Story</i>" : ""}</div>`).join("")}</article>`).join("")}</div>`;
}
...
function injuries(analysis: SliceAnalysis): string {
  ...
  return `<div class="injuries">${analysis.injuryArcs.map((arc) => `<article><b>${html(wrestlerName(analysis, arc.wrestlerId))}</b><span>Injuries: ${arc.injuryTicks.map((tick) => `W${week(analysis, tick)}`).join(", ") || "—"} · Missed: ${arc.missedShowTicks.map((tick) => `W${week(analysis, tick)}`).join(", ") || "—"} · Returns: ${arc.returnTicks.map((tick) => `W${week(analysis, tick)}`).join(", ") || "—"}</span></article>`).join("")}</div>`;
}
...
function trajectoryRows(analysis: SliceAnalysis): string {
  return analysis.trajectories.slice().sort((a, b) => (b.end - b.start) - (a.end - a.start)).map((trajectory) => {
    ...
    return `<tr data-name="${html(trajectory.wrestlerName.toLowerCase())}"><td>${html(trajectory.wrestlerName)}</td><td>${trajectory.start}</td><td>${trajectory.end}</td><td class="${change > 0 ? "up" : change < 0 ? "down" : "flat"}">${change > 0 ? "+" : ""}${change}</td><td><svg class="trajectory" viewBox="0 0 260 52" preserveAspectRatio="none" aria-label="Popularity trajectory for ${html(trajectory.wrestlerName)}" data-values="${html(trajectory.samples.join(","))}"></svg></td></tr>`;
  }).join("\n");
}
...
function seedPanel(run: SliceReportRun, index: number): string {
  ...
  return `<section class="seed-panel" id="seed-${index}" ${index === 0 ? "" : "hidden"}>
    ...
    <section><h2>Slice criteria</h2><div class="table-wrap"><table><thead><tr><th>Criterion</th><th>Strength</th><th>Result</th><th>Observed</th></tr></thead><tbody>${criteriaRows(analysis.criteria)}</tbody></table></div></section>
