...
function dashboardResult(popularity: PopularityTuning, seedCount: number) {
  const result = runTuning(scenario, popularity, seedCount);
    ...
    runs: result.runs.map(({ seed, analysis }) => ({
      ...
      wrestlers: analysis.trajectories.slice().sort((a, b) => a.wrestlerName.localeCompare(b.wrestlerName)).map((trajectory) => ({
        ...
        popularityLog: (analysis.popularityLogs[trajectory.wrestlerId] ?? []).map((entry) => ({
          ...entry,
          week: Math.floor(entry.tick / (scenario.config.decisionTicksPerWeek + 1)) + 1,
...
const server = createServer(async (request, response) => {
  try {
    ...
    if (request.method === "GET" && request.url === "/api/config") {
      return send(response, 200, JSON.stringify({ scenario: scenario.manifest.name, popularity: scenario.config.popularity }), "application/json");
    }
    if (request.method === "POST" && request.url === "/api/run") {
      const body = await requestJson(request) as RunRequest;
      const popularity = popularityTuningSchema.parse(body.popularity);
      const seedCount = body.seedCount === 3 ? 3 : 1;
      return send(response, 200, JSON.stringify(dashboardResult(popularity, seedCount)), "application/json");
    }
