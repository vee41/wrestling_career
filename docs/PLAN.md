...
**As built (2026-08-06):** `assignPositions` now places the three distinguished positions greedily under two rules beyond heat: on television a match candidate carries `tvMainEventMatchBias` (30) for the main event only — additive, so a hotter angle can still close a show — and any program repeating the position it held on the previous show pays `repeatPlacementPenalty` (25). The discretionary half of the composer is score-selected: the planned-beat, stale-contender, peaking-blowoff and title passes stay reservation-based, and the story and rotation passes now build candidate slots, rank them by `scoreComponents`, and commit in score order. Each candidate's `selection` is recorded on the trace (`reserved` / `scored_story` / `scored_rotation`, new in `bookingCandidateTraceSchema`) together with every rejected candidate's own score, so "why was this booked?" is answerable next to what was not. booking_ai §10's missing `repeat-pairing penalty` is implemented (`repeatPairingPenalty` 20, `consecutivePairingPenalty` 60) and applied twice: as a soft-score term, and as the tiebreak in a new greedy rotation pairing that replaced adjacency in the due-order list — adjacency was quietly handing the same two acts each other because the rest and appearance rules put them side by side every week. `promotionObjectiveFit` gained `objectiveSlotFit`: `rebuild_championship` scores the *belt on the slot* rather than a flat 10 per reasonably popular participant (the source of the observed constant 20.0), and `prepare_major_event` finally means something — a program beat is worth `objectiveSlotFitBonus` (20), doubled on the new `isGoHomeShowTick`. The legacy `bookingObjective` / `bookingObjectiveSince` / `rotateBookingObjectiveIfDue` are deleted from contracts, sim and the tick pipeline (`schemaVersion` 8 → 9, fixtures migrated); `gmObjective` is the only creative direction, read by `objectiveFit`, every slot's `gmIntent`, and `interactions.ts`. Rotation form is exposed as `rotationForm` in the new [form.ts](../packages/sim/src/form.ts) — a derived last-N read over open-rotation results, no stored streak state — and the planner consumes it as a `contenderForm` term worth `contenderFormBonus` (8) per net win to `establish_challenger`/`elevate_act` candidates. That term is also what finally makes `fallbackObjective` reachable: `planPrograms` now ranks *both* candidates per story and takes the better one, and the alternative objective wins 1–2 programs per 26-week seed instead of being scored, traced and structurally unselectable. Two latent defects surfaced by making the score select: `freshness` read an absent appearance history as *least* fresh, which starved three wrestlers out of a single match across 26 weeks until it was changed to read "never booked" as maximally fresh; and `buildTemplates` now truncates an archetype from the *end* rather than the front, because every archetype opens on the eventual payoff winner and a one-show build followed by a payoff named the same wrestler twice. Seven knobs, all scenario-owned. Four metrics carry the phase in both report formats: `televisionMatchMainEvents`, `repeatedPlacements`, `scoreInversions`, `programsByObjective`.

**Measured.** 8-week/2-seed: **TV main events that are matches 1/6 → 4/6 on both seeds** (the two exceptions are the cycle-opening weeks, where a freshly created program has only a promo to offer); repeated card placements 10 → 2 and 5 → 2; score inversions **0**; PLE build coverage 57%/55% → 70%/40%; programs resolved 5/6 → 7/6; distinct main-eventers 7/9 → 9/10; TV match beats 3/4 → 6/5; direct rematches 3/3 → 4/3. At 26 weeks/3 seeds: **TV match main events 4/6/5 of 20 → 12/13/9 of 20**, **repeated placements 27/14/22 → 4/6/3**, score inversions **0/0/0**, no wrestler unbooked on any seed, and the alternative creative objective is selected on every seed. **MUST failures 2/3/2 (7) → 2/3/4 (9), and SL-10 holds** (it broke to 1 distinct #1 act before the `freshness` fix and recovered to 2 after). The two new failures are both one unit from passing on seed 3 — SL-1 at 2 risers instead of 3, and SL-9 at 2 matches instead of 3 for the same legend seed 2 already failed on. Neither was bought back with a knob, per the seed-noise rule 3.12.5 established. See [playtest-notes.md](playtest-notes.md).

**Three things to hand on.** (1) **3.12.5's SL-9 diagnosis was wrong, and the real cause is 3.12.9's.** The hand-on said the rotation pass's `restPenalty` was keeping Damian Priest off cards it had room for. He is a `legend`, and `storyGated` roles are excluded from open rotation *by design* — `restPenalty` never applies to him. His match count tracks his program count exactly (5 programs → 7 matches on seed 1; **1 program → 2 matches** on seeds 2 and 3), so SL-9 here is "the director never built a story around this legend", which is [3.12.9](#phase-3129--story-variety-post-payoff-life-and-the-long-horizon--before-26-week-tuning) task 1. Do not fix it by letting rotation spend a rare appearance. (2) **The story-slot budget is now the binding card-shape constraint, and it is still an inline `Math.min(2, targetSlotCount)` compared against the *total* slot count.** On a television show that already reserved two program beats it admits no story slots at all, which is why PLE build coverage moves so much between seeds. It should become a real knob with its own semantics (story items per show, not slots-so-far) when 3.12.8 adds title programs to the same pool. (3) **A deviation at the payoff records no revision, and that is correct** — `resolvePayoff` closes the plan before `replanForExecutionDeviation` reads it, so a feud that is over has no intent left to revise. `injury.test.ts` asserted the unconditional invariant and only passed because no payoff had deviated on those seeds; it now exempts ended plans explicitly. Anything that later wants a post-payoff creative response should hook the aftermath (3.12.9 task 3), not the deviation.
